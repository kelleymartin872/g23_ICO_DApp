...
describe('ICO', function () {
  ...
  describe("Deposit", function () {
    it('should have the correct soft cap, hard cap, minimum and maximum purchase amounts and time limits', async function () {
      ...
      expect(await ico.startTime()).to.equal(1651987200);
      expect(await ico.endTime()).to.equal(1652073600);
    });
  
    it('should allow users to deposit ether', async function () {
      // await ethers.provider.send('evm_increaseTime', [-42000]);
      // const timestamp = Date.parse("5/8/2023 00:00:00 GMT") / 1000;
      const timestamp = Date.parse("5/9/2023 00:00:00 GMT") / 1000;
    ...
    it('should not allow users to deposit ether outside the time limits', async function () {
      // await ethers.provider.send('evm_increaseTime', [-42000]);
      await expect(
        await ico.connect(addr1).receive({value: ethers.utils.parseEther('0.02')})
    ...
    it('should not allow users to deposit ether below the minimum purchase amount', async function () {
      await expect(
        await ico.connect(addr1.address).deposit({value: ethers.utils.parseEther('0.005')})
    ...
    it('should not allow users to deposit ether above the maximum purchase amount', async function () {
      await expect(
        await ico.connect(addr1.address).deposit({value: ethers.utils.parseEther('0.1')})
    ...
    it('should not allow users to deposit ether if the hard cap has been reached', async function () {
      ...
      await expect(
        await ico.connect(addr1.address).deposit({value: ethers.utils.parseEther('0.02')})
  ...
  describe("Withdraw", function () {
    it('should allow users to withdraw their deposits if the soft cap has not been reached', async function () {
      await ico.connect(addr1).sendTransaction({ value: ethers.utils.parseEther('0.05') });
  
      await ethers.provider.send('evm_increaseTime', [86400]);
  
      const balanceBefore = await ethers.provider.getBalance(addr1.address);
  
      await ico.connect(addr1.address).withdraw();
    ...
    it('should not allow users to withdraw their deposits if the soft cap has been reached', async function () {
      ...
      await expect(await ico.connect(addr1.address).withdraw()).to.be.revertedWith(
  ...
  describe("Claim", function () {
    it('should allow users to claim their tokens if the soft cap has been reached', async function () {
      ...
      await ico.connect(addr1).sendTransaction({ value: amount });
      await ethers.provider.send('evm_increaseTime', [86400]);
      await ico.connect(addr1.address).claim();
    ...
    it('should not allow users to claim their tokens if the soft cap has not been reached', async function () {
      ...
      await expect(await ico.connect(addr1.address).claim()).to.be.revertedWith(
